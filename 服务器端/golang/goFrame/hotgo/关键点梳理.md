...
### 优化点
1. 密码加密使用`bcrypt / argon2 / scrypt`这类专用哈希算法，并保证 `salt` 是每个用户唯一的（这点已做到，数据库表中每个用户的salt都是唯一的）。
2. 补充登录失败次数限制与账号锁定策略，防止暴力破解。
...
==使用AI生成页面时，AI会根据后端接口来生成前端接口调用，在`src/api`目录下，在分配权限时可以参考对应文件中的接口地址来分配权限。==
...
2. 二级菜单需要分配权限，因为二级菜单通常涉及到页面的访问，需要调用接口来获取数据等。
    ...
    - 如果页面中的其它按钮权限等不想单独分配，可以直接将按钮权限等分配到菜单上，那么给角色分配了菜单权限后，角色也就有了这些按钮等的权限了。前端添加了权限控制的按钮也会都显示出来。
3. 按钮权限要分配它触发的接口的权限，并且前端可以根据权限地址来决定按钮是否显示。
    ...
    - 如果按钮点击的时候会弹出弹窗，弹窗会调用一个接口，那么可以将这个接口的权限添加到按钮上，给角色授予了按钮权限也就有了这个接口的权限，用户就可以点击按钮并且弹窗能正常显示。
...
后端：后端在 `internal\router\admin.go` 中调用了 `AdminAuth` 中间件，用于判断用户是否有某个接口权限。
```go
...
项目采用了Casbin作为RBAC（基于角色的访问控制）框架，主要包含以下组件：

| 组件 | 文件 | 职责 |
|------|------|------|
| **Enforcer** | `enforcer.go` | Casbin核心实例，管理权限策略和验证 |
| **Adapter** | `adapter.go` | 自定义数据库适配器，策略持久化 |
| **Model配置** | `casbin.conf` | 定义访问控制模型规则 |

#### 二、权限模型配置 (`casbin.conf`)

```ini
[request_definition]
r = sub, obj, act    # 请求：角色(subject)、资源(object)、操作(action)
...
[policy_effect]
e = some(where (p.eft == allow))  # 只要有一条策略允许就通过
...
**匹配逻辑说明**：
- `g(r.sub, p.sub)`：检查用户角色是否匹配策略中的角色（支持角色继承）
- `keyMatch(r.obj, p.obj)`：支持URL路径通配符匹配（如 `/api/v1/*`）
- `regexMatch(r.act, p.act)`：支持请求方法正则匹配（如 `GET|POST`）
...
**权限加载逻辑**：
- 关联查询 `admin_role`（角色表）、`admin_role_menu`（角色菜单关联表）、`admin_menu`（菜单表）
- 筛选启用状态的角色和菜单
- 排除超级管理员（超级管理员自动拥有所有权限）
- 将菜单权限标识转换为Casbin策略规则 `[role_key, permission, action]`
...
1. 获取当前登录用户信息
2. 超级管理员直接放行
3. 普通用户调用 `Enforcer.Enforce(roleKey, path, method)` 验证

#### 五、策略管理API

| 方法 | 功能 | 说明 |
|------|------|------|
| `Clear(ctx)` | 清空所有策略 | 递归清理，确保完全删除 |
| `Refresh(ctx)` | 刷新权限策略 | 先清空再重新加载 |
| `AddPolicy()` | 添加单条策略 | 运行时动态添加 |
| `RemovePolicy()` | 删除单条策略 | 运行时动态删除 |

#### 六、数据库适配器 (`adapter.go`)

自定义适配器实现了Casbin的 `persist.Adapter` 接口，支持：
- **策略持久化**：将策略规则存储到 `admin_role_casbin` 表
- **多数据库支持**：MySQL和PostgreSQL
- **批量操作**：支持批量增删改策略

#### 七、使用示例

```go
// 添加权限策略（角色 admin 可以访问 /api/*）
casbin.Enforcer.AddPolicy("admin", "/api/*", "GET|POST")

// 验证权限
ok, _ := casbin.Enforcer.Enforce("admin", "/api/user/list", "GET")
...
策略表 `admin_role_casbin` 字段说明：

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | int64 | 主键 |
| `p_type` | string | 策略类型（p/g） |
| `v0` | string | 参数0（角色标识） |
| `v1` | string | 参数1（权限/资源路径） |
| `v2` | string | 参数2（请求方法） |
| `v3-v5` | string | 扩展参数 |

#### 九、设计特点

1. **超级管理员豁免**：超级管理员不受Casbin规则限制，直接放行
2. **动态刷新**：角色或菜单变更后调用 `Refresh()` 即可更新权限
3. **通配符支持**：使用 `keyMatch` 支持URL路径通配符（如 `/api/v1/*`）
4. **请求方法灵活匹配**：使用正则匹配支持多种HTTP方法组合
5. **数据库无关性**：通过适配器抽象，支持多种数据库类型
...
### 后端路由配置
后端路由是在`internal/cmd/http.go`中进行配置的，如：
```go
...
```
**注：API接口主要在route.Admin这个路由中，router.WebSocket会在系统监控中用到，其它的可以根据需要注释掉，以减少资源占用。**

然后在`internal/router/admin.go`中进行具体的路由注册。这个文件中一些自带的路由也是没用到的，可以根据需要注释掉，以减少资源占用。
新增了路由也在这个文件中进行添加，注意 middleware 的调用，根据需要添加到合适的位置。

#### Api路由
`router.Admin(ctx, group)`中注册了后台路由，`router.Api(ctx, group)`中注册了API路由。API路由是给前台应用调用的（非后台管理系统，如官网、小程序等），前台应用的接口需要与后台管理系统的接口区分开来，避免权限控制、数据泄露等问题。

## 中间件
hotgo的中间件是编写在`logic`中的，在`internal\logic\middleware`目录中编写中间件，然后运行`gf gen service`生成了相应的service文件`internal\service\middleware.go`，最后通过`service.Middleware().Xxx`来调用中间件。如：
```go
...
## 手写API
### 自动生成controller文件
编写的API需要写在`api/api`目录中并且加上版本号，才能使用`gf gen ctrl`生成对应的controller文件。hotgo的`hack/config.yaml`中单独配置了`ctrl`命令的生成规则。
**自动生成要配合版本号这些，后台管理系统这样的项目使用起来还是有点麻烦，感觉不使用自动生成，自己手写可以简化项目架构。**

### 手写controller文件（含整套流程）
* 数据库有变动需要先运行`gf gen dao`。
* 纯手写没有版本控制，对于要提供给APP等的接口可能会造成麻烦。另外建议APP端做好接口使用的统计，以便后续接口变动时能知道哪些接口需要修改。

#### 1. 编写api和model
在`api/mapi`下创建api文件，如`device/device.go`，按照goframe的规则编写。
hotgo中将请求和响应的结构体都单独声明放在了`model`目录中，为了和自动生成接口的model区分开，手写接口的model放在了`model/apim`目录中，如`model/apim/device.go`。然后在`api/mapi/device/device.go`中引入这个model包，并在接口声明中使用这些结构体，如：
```go
type HistoryLocReq struct {
    g.Meta `path:"/device/historyLoc" method:"get" tags:"设备列表" summary:"查询设备历史位置"`
    apim.DeviceHistoryLocInp // 使用了 model 中定义的结构体
}
```
把请求/响应结构体抽离出来成为单独的model，有些地方可以进行复用。我们自己编写的model包名为apim，和自动生成的model区分开。

#### 2. 编写controller
在`internal/controller`下创建`apic`目录，以区分由我们自己编写的controller，然后在`apic`目录下创建对应的controller文件，如`device.go`，编写controller逻辑。（路径为`internal/controller/apic/device.go`。）
由于没有使用自动生成，我们就需要先声明一个struct，用于添加方法和添加路由。如：
```go
// 我们自己编写的controller包名都为apic
package apic
...
// 声明一个空的struct
type cDevice struct{}

// 使用struct创建一个全局变量，供路由调用
var Device = cDevice{}

// 给struct添加路由执行方法
func (c *cDevice) Delete(ctx context.Context, req *device.DeleteReq) (res *device.DeleteRes, err error) {
	err = service.SysDeviceData().Delete(ctx, &req.DeviceDeleteInp)
	return
}
```
...
在`logic`目录下创建`apil`目录，以区分由我们自己编写的logic，然后在`apil`目录下创建对应的logic文件，如`device.go`，编写logic逻辑。（路径为`logic/apil/device.go`。）
然后按照goframe的规则编写logic，注意包名统一使用apil，然后运行`gf gen service`命令生成对应的service文件，最后在logic的init方法中注册service。

**我们自己编写的controller包名为apic，model包名为apim，logic包名为apil。**

#### 4. 注册路由
在`internal/router/admin.go`中使用controller中创建的全局变量`Device`来注册路由，如：
```go
apic.Device,
// device.NewV1(), // 带版本的自动生成接口路由的注册方式
```

## 其它
### 路由别名
在菜单配置中有个路由别名配置项，作用是在页面跳转的时候可以通过别名来进行跳转。
```ts
router.push({ name: 'deviceList' }) // 通过路由别名来跳转
```
按钮这些其实可以随便取一个唯一的别名就行，使用AI编程实际上感觉别名也不重要了。

### 上传驱动配置
`Chooser.vue` 本身不配置上传驱动。它只做两件事：

1. 用 `config_upload_drive` 字典渲染“上传驱动”筛选项。
2. 调 `/attachment/list` 按 `drive` 查询已上传附件。

真正决定上传到哪里的是后端全局上传配置：`uploadDrive`。上传弹窗 [Upload.vue](D:/Learn/hotgo/web/src/components/FileChooser/src/Upload.vue) 固定请求 `/upload/file`，只传 `uploadType`，没有传 `drive`。

**驱动链路**
- 前端上传：`web/src/components/FileChooser/src/Upload.vue`
- 上传配置页：`web/src/views/system/config/UploadSetting.vue`
- 后端上传入口：`server/internal/library/storager/upload.go`
- COS 实现：`server/internal/library/storager/upload_cos.go`
- 配置模型：`server/internal/model/config.go`

支持的驱动值在 [upload.go](D:/Learn/hotgo/server/internal/consts/upload.go)：
...
1. 进入后台：系统设置 → 配置管理/上传配置。
2. 默认驱动选择：`腾讯云`，实际值是 `cos`。
3. 填腾讯云 COS 标签页：
   - `uploadCosSecretId`：腾讯云 API `SecretId`，不是单纯 APPID。页面 label 写“APPID”有误导，代码里实际传给 COS SDK 的是 `SecretID`。
   - `uploadCosSecretKey`：腾讯云 API `SecretKey`。
   - `uploadCosBucketURL`：Bucket 访问域名，例如：
     ```text
     https://your-bucket-1250000000.cos.ap-guangzhou.myqcloud.com
     ```
     不建议末尾带 `/`。
   - `uploadCosPath`：对象存储内路径，例如：
     ```text
     hotgo/attachment/
     ```

保存后，`server/internal/logic/sys/config.go` 会同步调用 `storager.SetConfig(upload)`，后续上传会走：

```go
New(config.Drive).Upload(ctx, file)
```

当 `uploadDrive = cos` 时，会进入 `CosDrive.Upload()`，把文件上传到腾讯云 COS，并写入附件表。最终访问 URL 拼接逻辑是：

```go
config.CosBucketURL + "/" + fullPath
```

所以你的 COS Bucket 需要满足：
- 后端服务器能用 `SecretId/SecretKey` 写入对象。
- `uploadCosBucketURL` 能被浏览器访问。
- 如果 Bucket 是私有读，当前代码不会生成临时签名 URL，预览/下载会失败；需要配置公有读、CDN 公有域名，或改造为签名访问。

还有一个限制：当前 COS 驱动不支持分片上传，`CreateMultipart` 和 `UploadPart` 会直接返回“当前驱动暂不支持分片上传！”。普通 `FileChooser` 上传没问题，但如果使用 `multipartUpload.vue` 上传大文件，COS 需要额外实现分片逻辑。

### model 中的 Filter
model 中的 Filter 函数通过全局中间件调用：`service.Middleware().PreFilter`，它的职责是参数清洗、数据校验等。而不是 logic 的功能。所以 model 中可以写 Filter 函数这个设计是合理的。

这样做的好处是：
- controller 保持薄，只透传参数。
- logic 拿到的 in 已经是清洗后的数据。
- 多个入口复用同一个入参时，不容易漏校验。
- 校验错误能在请求早期返回，避免进入业务流程。


## 注意点
- 超级管理员在前端源码中有些地方是通过`id === 1`或`id !== 1`来判断的，所以超级管理员的用户id必须为1。如`组织管理-后台用户`里的操作按钮。
    (不过分析了下主要是对id为1的超管进行了一些按钮的隐藏，如果创建了另一个超管用户，他除了不能对id为1的超管用户进行一些操作外，其它功能都能对标id为1的超管)
    ```ts
    function getDropDownActions(record: Recordable): ActionItem[] {
        // 超级管理员那一行没有按钮，直接返回空数组
        if (record.id === 1) {
            return [];
        }
    ...
    ```
- hotgo数据库中的时间字段都使用的`timestamp`类型，需要修改为`timestaptz`，否则会有时区问题导致时间不对。可执行以下SQL命令进行一键修改：
    ```sql
    DO $$
    DECLARE
        r RECORD;
