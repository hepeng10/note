[wangdoc/bash-tutorial](https://github.com/wangdoc/bash-tutorial/tree/master/docs)
[sourcegraph 版](https://sourcegraph.com/github.com/wangdoc/bash-tutorial/-/tree/docs)
...
touch [文件名]  用于修改文件的时间属性，如果文件不存在则会创建文件。
...
rm -rf test  // 递归删除test目录，使用 r 递归删除目录中的所有文件；另外 f 也是必须的，如果不加 f 则会对目录里的每个文件进行询问是否确认删除
