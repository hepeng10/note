...
int main(void) {
    ...
    // %zd 是 sizeof 运算符的转换说明，用于表示 size_t 类型
...
int main(void) {
    char c = 'A'; // 基本类型 char。单引号
    char s[40] = "这是个字符串"; // 派生类型 char 数组。双引号
    printf("%c\n", c); // 使用 %c 输出字符
    printf("%s\n", s); // 使用 %s 输出字符串
    printf("%d\n", c); // 会输出 A 对应的 ASCII 码
...
**Unicode 编码中的中文等字符存储在内存中也是二进制数字，一个汉字对应一个数值，3个字节足以表示各种语言的文字了。（编译器根据源文件的编码方式解释字符。例如，如果源文件是以UTF-8编码保存的，编译器会根据UTF-8的规则解释每个字符。UTF-8中，每个中文字符的前几个比特位会形成特殊的模式，标识这是多字节字符的一部分。）**
```c
// 错误
int main() {
...
#### UTF-8 编码下的多字节字符识别
UTF-8 是一种可变长度的字符编码，使用 1 到 4 个字节表示 Unicode 字符。它通过字节的前几位来区分是单字节还是多字节字符，且前导字节和后续字节具有特殊模式：

* 单字节字符（ASCII）：1 个字节，最高位是 `0`，即范围是 `0x00` 到 `0x7F`（对应标准 ASCII 字符）。
* 多字节字符：
  * 2字节字符：前导字节以 `110` 开头，后续字节以 `10` 开头。比如 `110xxxxx 10xxxxxx`。
  * 3字节字符：前导字节以 `1110` 开头，后续字节以 `10` 开头。比如 `1110xxxx 10xxxxxx 10xxxxxx`。
  * 4字节字符：前导字节以 `11110` 开头，后续字节以 `10` 开头。比如 `11110xxx 10xxxxxx 10xxxxxx 10xxxxxx`。

举例来说，`你`字在 UTF-8 中的编码是 `0xE4BDA0`，对应的二进制形式是 `11100100 10111101 10100000`，因此编译器可以通过第一个字节 `1110` 知道这是一个 3 字节的字符。**当然，编译器首先要从代码中声明的类型，知道这是一个字符串类型，才会使用此规则进行解析。**
...
int main(void) {
    ...
    int value2 = ADD * 3; // 替换后就会变成 1+2*3，结果不合预期。所以在定义 ADD 时，应该使用括号将表达式括起来。
    return 0;
}
```
...
# 运算
### 除法
c 语言中，整数除法返回的也是整数，这就导致整数相除产生的小数部分会被丢弃，所以在会使用到除法时，应该将变量定义为 float 或 double 等浮点数类型，而不应该定义为 int 这样的整数类型。
```c
#include <stdio.h>
int main(void)
{
...
```
C 语言规定，sizeof 返回 size_t 类型的值。而 size_t 并不是个新类型，而是某个标准类型的别名，使用 typedef 定义。C 头文件系统可以使用 typedef 把 size_t 作为 unsigned int 或 unsigned long 的别名。这样，在使用 size_t 类型时，编译器会根据不同的系统替换标准类型。我们可以使用 %zd 转换说明来表示接受 size_t 类型。
...
### 什么是真
在 C 语言中，1表示真，0表示假。实际上非0的整数都是真，无论正负。而浮点数放到条件语句中编译器会给出警告，即使能作为真值继续运行。
```c
#include <stdio.h>
int main(void) {
    if (1) {} // 正常运行，为真
...
### 尾递归
最简单的递归形式是把递归调用置于函数的末尾，即正好在 return 语句之前（也就是 return 递归函数的调用）。这种形式的递归被称为尾递归（tail recursion），因为递归调用在函数的末尾。尾递归是最简单的递归形式，因为它相当于循环。
...
```c
// 尾递归版本
unsigned long Fibonacci_tail(unsigned n, unsigned long a, unsigned long b)
{
    if (n == 0)
        return a;
    if (n == 1)
        return b;
    return Fibonacci_tail(n - 1, b, a + b);  // 递归调用是最后一步，没有额外操作
}
```
...
// 这种方式声明字符串，末尾必须加 \0，否则程序不知道这个字符串的结尾
const char s3[] = {'A', 'B', 'C', 'D', '\0'};
```
...
3. 按位或：|
    ...
    或的全称是 inclusive OR，表示“包含性的或”，即只要两个位中有一个为1，结果就为1。
    ```c
    (10010011) | (00111101) // 表达式
    (10111111) // 结果值
    ```
4. 按位异或：^
    ...
    异或 Exclusive OR 实际意思是“排他性的或”，排除了两个位相同的情况，即进行运算的两个位要不同，结果才为1。
    ```c
    (10010011) ^ (00111101) // 表达式
    (10101110) // 结果值
    ```
...
```
通常项目中有很多 c 文件，要一起编译可以进到项目目录中使用：
```sh
gcc *.c -o myApp
```
...
整个程序由 list.h（定义数据结构和提供用户接口的原型）、list.c（提供函数代码实现接口）和 films.c（把链表接口应用于特定编程问题的源代码文件）组成。要运行该程序，必须把 films.c 和 list.c 一起编译和链接，如： `gcc films.c list.c`。
```c
