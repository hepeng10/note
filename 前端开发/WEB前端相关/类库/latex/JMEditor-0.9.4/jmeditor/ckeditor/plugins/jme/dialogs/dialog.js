...
document.write(
	"<link href=\"" + JMEditor.jmeBasePath + "mathquill-0.10.1/mathquill.css\" rel=\"stylesheet\" type=\"text/css\" />" +
	"<script type=\"text/javascript\" src=\"" + JMEditor.jmeBasePath + "jquery-1.8.3.min.js\"></script>" +
	"<script type=\"text/javascript\" src=\"" + JMEditor.jmeBasePath + "mathquill-0.10.1/mathquill.min.js\"></script>" +
...
var jmeMath = [
	[
		"\\\\left \\\\{  \\\\right \\\\}", "\\\\begin{cases} & \\\\text{ if } x= \\\\\\\\ & \\\\text{ if } x= \\\\\\\\ & \\\\text{ if } x= \\\\end{cases}", "x^{}", "x_{}", "x^{}_{}", "{/}bar{}", "{/}sqrt{}", "{/}nthroot{}{}",
...
function jmeInit() {
	...
	//公式编辑框
	$("#jme-math").html("").css("font-size", JMEditor.defaultFontSize);
	// MQ.StaticMath($("#jme-math")[0]);
	window.mathField = MQ.MathField($("#jme-math")[0]);
	//验证版本信息
