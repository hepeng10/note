...
# ScrollView
### ScrollView 内置节流
ScrollView 组件内置节流，原生小程序可通过设置隐藏 props `throttle="{{ false }}"` 来关闭节流，但是 taro 中没有支持此项。所以需要通过 `@tarojs/plugin-inject` 插件来添加 props。
```ts
...
```
需要注意的是，ScrollView 在使用时默认就会有了这个 props，组件也增加了 throttle 这个 props，可以手动修改：
```jsx
<ScrollView
  ...
  enableFlex
  refresherEnabled
  refresherTriggered={refresherTag}
  onScroll={onScroll}
  onRefresherRefresh={pullRefresh}
  onScrollToLower={onScrollToLower}
  scrollIntoView={scrollIntoView}
  lowerThreshold={400}
  throttle={true} // 新增的 props，手动修改
>
```
### ScrollView 自适应高度
当使用 flex 布局时， scrollView 设置为 `flex: 1` 并不能成为自适应高度的容易，需要添加 css 样式 `overflow-y: auto;` 才能自适应高度滚动。
...
### catchMove 和 ScrollView
在页面布局时，可以正常的一直往下编写内容，这时页面也会拥有滚动功能。但是，这时如果我们需要弹出一个 Modal，在这个 Modal 上绑定 catchMove，并不能阻止页面的滚动。所以，当需要编写滚动的页面时，应该使用 ScrollView 组件，给组件添加 scrollY 属性进行滚动，这时在 Modal 上添加 catchMove 就能阻止 ScrollView 组件的滚动了。

### ScrollView 自动回到顶部
和 ScrollView 同级节点更新时会触发 ScrollView 重新渲染，从而回到顶部。和 ScrollView 同级节点最好使用 View 再包一层，避免同级节点更新。
https://github.com/NervJS/taro/issues/10298#issuecomment-920539213
...
# useDidShow
1. 当从一个页面返回或跳转到另一个页面的时候，我们可能会有在组件销毁时操作 store 中某个数据，在下一个页面显示时获取这个数据进行一些操作的需求。我们可能就会在 useDidHide 或 useEffect 的 return 中操作数据，下个页面的 useDidShow 中获取数据。但是这时就有个问题，安卓和 iOS 在销毁和展示的运行时机不同，安卓可能先运行 useDidShow，再运行 useDidHide，iOS 可能先运行 useDidHide 再运行 useDidShow，这时安卓机就出问题了，不能获取到上个页面销毁时赋予的最新值。所以这个问题我们应该不使用 useDidShow，而改用 useEffect 并监听对应 store 数据的变化，这样就能保证拿到最新的值了。

2. 在子组件中使用 useDidShow 时，如果父组件通过逻辑判断是否渲染子组件的情况，那么可能一开始是 false 的，子组件不被渲染，导致 useDidShow 不被加载，当条件通过接口跟异步方法变成 true 后，子组件渲染了，useDidShow 也不会再执行了。这种情况可以通过 visible 参数传给子组件，子组件判断 props 来决定是否渲染，就可以保证子组件始终会加载，useDidShow 也就能保证执行了。
...
# iOS 日期转换
在 iOS 中使用 `new Date()` 进行日期转换时有兼容问题，安卓中支持 `new Date(2022-08-10)` 但是 iOS 中不支持 - 分割的日期，需要使用 `new Date(2022/08/10)`
...
首先使用 yarn 安装 ts-jest `yarn add --dev jest typescript ts-jest @types/jest`，文档：[ts-jest 官方文档](https://kulshekhar.github.io/ts-jest/docs/getting-started/installation)。
再安装 @testing-library/react，注意由于 taro 使用的 react 17，所以只能安装12版本，不能安装最新版本，`yarn add --dev @testing-library/react@12.1.5`。
还需要配置 jest.config.js 配置：
```js
/** @type {import('ts-jest/dist/types').InitialOptionsTsJest} */
module.exports = {
  preset: 'ts-jest',
  // testEnvironment: 'node',
  testEnvironment: 'jsdom', // 使用 jsdom 环境才能测试 dom
  verbose: true,
  moduleNameMapper: {
    // 将 taro 替换为自己的文件，在这个文件中将 @tarojs/taro-h5 和自己补充的 API 导出
    '^@tarojs/taro$': '<rootDir>/test/__modules__/@tarojs/taro',
    // 使用 h5 的组件，View 就会编译成 div
...
```
主要配置 testEnvironment 为 jsdom，moduleNameMapper 中我们将 @tarojs/components 映射为 h5 组件，这样我们在编写单元测试时实际是当做 h5 页面来测试的，View 组件就会编译成 div，就能在 jsdom 中运行了。

moduleNameMapper 的 key 是正则匹配，比如上面的  `'@tarojs/components'` 我们将其替换为 `'@tarojs/components/dist-h5/react'`，当我们 `import { View } from '@tarojs/components-h5'`，也会替换为 `import { View } from '@tarojs/components/dist-h5/react-h5'`。而我们可能只希望 `'@tarojs/components'` 替换，`'@tarojs/components-h5'` 不会替换，这时就要写成 `'^@tarojs/components$'`。

比如 `'<rootDir>/test/__modules__/@tarojs/taro'` 文件中的内容：
```ts
import taro from '@tarojs/taro-h5/dist/index';
export * from '@tarojs/taro-h5/dist/index';
...
```
我们引入 `@tarojs/taro-h5` 的相关 API 并添加小程序有而 H5 没有的 API 进行导出。这 `import taro from '@tarojs/taro-h5/src/api/taro'`，如果 moduleNameMapper 的 `'^@tarojs/taro$'` 改成 `'@tarojs/taro'`，就会导致前面的 import 变成 `import taro from '<rootDir>/test/__modules__/@tarojs/taro-h5/src/api/taro'`，这不是我们期望的。

# webpackChain
taro 的 ./config 中的配置，index 是公共配置，dev.js 和 prod.js 是开发和生产配置，但是 taro 的合并规则是使用 dev.js 或 prod.js 中的配置覆盖公共配置，类似：
```js
mini: {
  ..., // 公共配置
  ...dev.mini, // dev.js 中的 mini 配置会合并到公共配置中
}
```
这时，在 index.js 中配置了 webpackChain，又在 dev.js 中配置了 webpackChain，结果就是只会执行 dev.js 中的 webpackChain，所以不要认为两个文件都使用 chain.merge 合并的，为什么只会执行 dev.js 中的 webpackChain。

# 公众号二维码
小程序里可以展示公众号二维码，通过长按识别直接进入公众号。产品给出二维码后，通过浏览器扫描等手段可以转换成链接，链接配到后端数据库中通过接口返回，将链接使用文本转二维码的组件可以展示二维码。不过有个问题就是，某些将原二维码生成链接的功能，可能生成出的是 https 的链接，需要改成 http 才行。

# WebView 相关
### 动态修改 title
小程序中加载 H5 后，直接通过 document.title 修改 title，并不会更新小程序的 title，需要使用以下方式：
```js
...
# 订阅消息
订阅消息必须通过按钮点击触发，大部分机型在点击触发订阅消息时可以进行跳转，但是小部分机型会弹不出订阅消息。并且这种弹出订阅消息并跳转的方式不好封装背景中提示点击允许的操作，因为页面跳转会导致组件销毁，从而提示组件也就显示不出来了。

# 视频号
[【干货分享】如何玩转微信视频号直播接口？](https://developers.weixin.qq.com/community/develop/article/doc/0006c2671ac2a86e6e1d198dd51413)
1. 视频号在直播中，调用 reserveChannelsLive 无法唤起预约弹窗。
2. 预约的 status 只会返回0，因为取消或关闭预约都是走 fail 了。
3. 预约不能通过 status 或 reservable 判断用户是否预约，只能自己处理，如保存本地或服务器。
4. 预约弹窗只能在真机上测试

# 地图
* 开发者工具中，使用鼠标滚轮缩放地图不会触发 onRegionChange 时间，真机上可以。
* 拖拽时会有惯性滑动，开发者工具中 onRegionChange 的 end 事件会在鼠标放开时就执行，真机上会在惯性滑动结束时执行。

# 广告
一个页面的广告只能是1个，两个就会被关闭广告，所有广告都不能显示。

# Swiper
做那种滑动后需要重置索引实现无限滑动的 Swiper 时，使用原生 Swiper 才不会产生重置时的闪动问题，taroify 的 Swiper 会闪一下。并且要将 key, current, swiper 数据放到一个 state 中，才能同时更新，不然也会导致闪动。
原神 Swiper 没有禁止用户滑动的功能，需要在 SwiperItem 内添加容器，设置 width: 100%, height: 100%，和 catchMove

# 打包体积优化
### 配置
#### optimizeMainPackage 配置
```js
mini: {
  optimizeMainPackage: {
    enable: true,
...
```
taro 文档里写的这个配置可以优化主包体积，目前 taro 已经默认开启了。
#### terser 压缩
```js
mini: {
...
```
prod.ts 的配置中 添加 terser 压缩功能可以减少一些体积
### 按需导入
使用的第三方库如 lodash, taroify 等，可以使用 `babel-plugin-import` 进行配置按需导入。
### 统一导出优化
```ts
export { default as AddWx } from './addWx';
export { default as BackTop } from './backTop';
export { default as Buoy } from './buoy';
export { default as Button } from './button';
export { default as Checkbox } from './checkbox';
export { default as ChooseModal } from './chooseModal';
export { default as Empty } from './empty';
export { default as ErrorTip } from './errorTip';
export { default as Filter } from './filter';
export { default as FixedNav } from './fixedNav';
export { default as HeadPic } from './headPic';
export { default as JgjTips } from './jgjTips';
export { default as JinGang } from './jinGang';
export { default as JobItem } from './jobItem';
export { default as ListItem } from './listItem';
```
项目中这样统一到处组件会导致没法按需加载，只在分包中使用的也会被打包到主包，所以最好不要这样写。如果这样写了需要使用 `babel-plugin-import` 进行修改引入方式：
```js
// @/components 目录进行按需加载
[
  'import',
  {
    ...
    libraryDirectory: '',
    /* 
    * 组件不是以 default 导出的才需要设置为 false，默认为 true
    * 为 false 时编译后为：import { AddWx } from '@/components/addWx'
    * 为 true 时编译后为：import AddWx from '@/components/addWx'
    */
...
### 使用网络字体图标
小程序可以使用wx.loadFontFace方法来远程加载字体图标。
```js
wx.loadFontFace({
  family: 'my-iconfont',
...
```
[网络字体图标使用教程](https://blog.csdn.net/weixin_42657666/article/details/131453485)