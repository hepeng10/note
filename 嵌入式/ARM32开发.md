...
> 文中的很多数据要以具体芯片为准，需要查看芯片手册。文中列出的数据通常是教程中使用的芯片数据，如F103等。
...
# 寄存器
寄存器是单片机内部的一种特殊的内存，可以实现对单片机各个功能的**控制**。单片机经常会去读写寄存器的值，来判断某个功能当前的运行情况或修改某个功能当前的运行情况。
每个寄存器其实就很像是程序里的switch类型变量，有0/1两个值，寄存器中的每一位的地址是固定不会变的。而程序中的变量是根据程序运行情况分配的内存地址，会变化。

### 寄存器地址
寄存器的地址是当前模块的基地址加上偏移地址，如：
```c
// 假设 GPIOA 的基地址是 0x40010800，ODR 寄存器的偏移量是 0x0C
*(uint32_t *)(0x40010800 + 0x0C) = 0xfffe;
```
基地址就是当前模块的起始地址，可以在芯片手册的内存映射表中找到。偏移量可以在每个分类下看，也可以在总表看。
需要注意的是，0x40010800是GPIOA的基地址，而GPIO是属于APB2总线，而APB2总线的基地址是0x40010000，所以GPIOA的基地址是APB2基地址加上GPIOA的偏移量0x0800得到的。APB2的基地址又是基于总线矩阵的基地址0x40000000加上APB2的偏移量0x10000得到的。总线的基地址是芯片设计时确定的，具体可以看芯片手册的系统架构图。
**寄存器地址计算过程：**
![图 44](assets/1765768413929.png)  


### 寄存器映射
寄存器映射是将寄存器的地址映射为一个结构体，方便我们通过结构体成员的方式访问寄存器。
结构体的内存占用是连续的，每一个寄存器占32位，也就是4个字节，所以每个寄存器的偏移量是上个寄存器偏移量+4。
```c
// 假设 GPIOB 的基地址是 0x40010800
#define GPIOB_BASE 0x40010800
// 定义 GPIO_TypeDef 结构体，包含寄存器成员
typedef struct {
    volatile uint32_t CRL;   // 端口配置低寄存器 偏移量0x00
    volatile uint32_t CRH;   // 端口配置高寄存器 偏移量0x04
    volatile uint32_t IDR;   // 端口输入数据寄存器 偏移量0x08
    volatile uint32_t ODR;   // 端口输出数据寄存器 偏移量0x0C
    volatile uint32_t BSRR;  // 端口位设置/清除寄存器 偏移量0x10
    volatile uint32_t BRR;   // 端口位清除寄存器 偏移量0x14
    volatile uint32_t LCKR;  // 端口配置锁定寄存器 偏移量0x18
} GPIO_TypeDef;
// 定义 GPIOB 指针，指向 GPIOB 的基地址
...
# MAP 文件
**主要用于分析代码进行优化，详细内容可以看正点原子的《STM32 MAP文件浅析》**
MDK编译过程中会生成很多种类型的文件，其中就有MAP文件，这个文件记录了程序中各个函数和变量在内存中的地址分布情况。**通过查看MAP文件，我们可以了解程序的内存使用情况，找出内存使用的瓶颈，优化内存布局等。**
* .o文件：可重定向对象文件，每个.c/.s文件都对应一个.o文件
* .axf文件：可执行对象文件，由.o文件链接生成，仿真的时候需要用到此文件
* .hex文件：INTEL Hex格式文件，用于下载到MCU运行，由.axf转换而来
* .map文件：连接器生成的列表文件，对分析程序存储占用情况非常有用
* 其它：.crf、.d、.dep、.lnp、.lst、.htm、.build_log.htm等一般用不到
...
# GPIO
GPIO是通用输入输出端口（General Purpose Input/Output）的简称，是单片机中最常用的外设之一。GPIO引脚可以配置为输入模式或输出模式，用于与外部设备进行数字信号的交互。
GPIO分为PA,PB,PC,PD等多个端口，每个端口包含多个引脚（Pin），ARM32通常每个端口是16个引脚，从PA0-PA15、PB0-PB15等。

### GPIO特点
1. 不同型号，IO口数量可能不一样，可通过选型手册快速查询
2. 快速翻转，每次翻转最快只需要两个时钟周期（F1超频后最高速度可以到50Mhz）
3. 每个IO口都可以做中断
4. 支持8种工作模式

### 电气特性
1. STM32工作电压范围：2V ≤ VDD ≤ 3.6V
2. GPIO识别电压范围：（COMS端口）
  逻辑0：-0.3V ≤ VIL ≤ 1.164V
  逻辑1：1.833V ≤ VIH ≤ 3.6V
  不确定：1.164V < V < 1.833V，可能是0也可能是1
3. GPIO输出电流：单个IO，最大25mA。总输出150mA，所以多个IO同时输出时，总输出电流不能超过150mA。如果要输出更大的电流需要使用电流放大电路，或控制继电器使用别的电流源。

### 引脚分布
引脚类型：电源引脚、晶振引脚、复位引脚、下载引脚、BOOT引脚、GPIO引脚
![图 53](assets/1768894185262.png)  
V开头的都是电源引脚；带OSC的是晶振引脚，如PC 14-OSC 32_IN；NRST是复位引脚；下载引脚有JTAG和SWD等；BOOT0和BOOT1是BOOT引脚；以P开头的是GPIO引脚。
IO引脚分布特点：按组存在、组数视芯片而定、每组最多16个IO引脚

### IO端口结构
![图 55](assets/1768898168266.png)  
...
|开漏式复用输出|	片上外设功能（硬件IIC 的SDA、SCL引脚等）
|推挽式复用输出|	片上外设功能（SPI 的SCK、MISO、MOSI引脚等）
#### 输入浮空
![图 56](assets/1768898486605.png)  
#### 输入上拉
![图 57](assets/1768898877998.png)  
#### 输入下拉
* 和输入上拉相反，空闲时IDR为1，IO引脚为低电平。
* 输入上拉和下拉类似于增加了默认值，当外部信号没有连接时，引脚的电平就是默认值。
#### 模拟功能
![图 58](assets/1768899038617.png)  
#### 开漏输出
![图 59](assets/1768900150666.png)  
* ODR写0时，**输出控制中的反向器进行反向后输出高电平**，N-MOS管导通后接地，输出低电平。（外部VDD电流也会流向N-MOS的接地）
* 外部VDD接5V就能输出5V电压，用芯片内部VDD无法输出5V，这是开漏输出的作用。
#### 开漏复用输出
和开漏输出类似，只是使用“复用功能输出（其它外设）”那条线路进行输出。
#### 推挽输出
![图 60](assets/1769047099349.png)  
* 要点亮一个LED灯，外部是3.3V，那么GPIO引脚就该设置为推挽输出模式，并且输出低电平，而不能用输入模式。推挽输出0的情况下，N-MOS管导通，电流通过GPIO引脚到达内部N-MOS那里的地，形成电流通路。而输入模式下引脚内部呈现高阻态，等于只是一个很大的电阻/电路断开，所以不能点亮LED灯。
  ![图 65](assets/1769244894878.png)  
  ==要驱动外部设备，需要使用输出模式。输入模式只能感知外部设备，如按键按下，MCU值负责读取电平，不给外部供电，不吸电流。==
#### 复用推挽输出
和推挽输出类似，只是使用“复用功能输出（其它外设）”那条线路进行输出。

#### F4/F7/H7系列和F1系列的GPIO差异点
1. F1在输出模式，禁止使用内部上下拉；F4/F7/H7在输出模式，可以使用内部上下拉（在保护二极管左侧）
2. 不同系列IO翻转速度可能不同

### 寄存器
#### 寄存器介绍
![图 61](assets/1769050230511.png)  
不同芯片寄存器名称略有不同，但大致是相同的。
* RCC->APB2ENR |= RCC_APB2ENR_IOPBEN; 时钟使能寄存器，这里是 GPIOB 的。
* GPIOx_CRL 端口配置低寄存器。配置输入/输出模式、速度、上下拉等。
  ![图 62](assets/1769050465756.png)  
* GPIOx_CRH 端口配置高寄存器。
  * ==每4位配置一个IO引脚如PA0，CRL有32位，所以可以配置8个IO引脚；CRH有32位也能配置8个IO引脚。所以最多配置16个IO引脚，所以一组引脚最多16个（如PA0-15）。0-7使用CRL配置，8-15使用CRH配置。==
  * 某些芯片不会分为两个寄存器，就是一个 GPIOx_CR 寄存器。
  * CNF配置为10-上拉/下拉输入模式，要配置具体的上拉还是下拉还需要配置ODR寄存器。
  * 模式配置补充：
    ![图 63](assets/1769061958640.png)  
  ```c
  // 配置一个GPIO引脚：GPIOA0
  GPIOA->CRL &= ~GPIO_CRL_MODE0; // 先将MODE0配置为00，输入模式
  GPIOA->CRL |= GPIO_CRL_CNF0_1; // 将CNF0的第1位置1
  GPIOA->CRL &= ~GPIO_CRL_CNF0_0; // 将CNF0的第0位置0。所以CNF就是10，上拉/下拉输入模式
  GPIOA->ODR |= GPIO_ODR_OD0; // 配置ODR的0位为1，上拉输入模式，默认是高电平
  // 同时配置两个 GPIO 引脚：GPIOB10和11
  GPIOB->CRH |= (GPIO_CRH_MODE10 | GPIO_CRH_MODE11); // 配置MODE10和11为11输出模式，最大速度50MHz
  GPIOB->CRH &= ~(GPIO_CRH_CNF10_1 | GPIO_CRH_CNF11_1); // 将CNF10和11的第1位置0
  GPIOB->CRH |= (GPIO_CRH_CNF10_0 | GPIO_CRH_CNF11_0); // 将CNF10和11的第0位置1。所以CNF就是01，通用开漏输出模式
  ```
* GPIOx_IDR 端口输入数据寄存器。获取引脚的输入值。一共16位，每个位对应一个引脚。
  ```c
  uint32_t idr = GPIOA->IDR; // 获取IDR寄存器的值（16个引脚一起的，但是需要32位存储，如果有uint16_t也可以）
  uint8_t id0 = idr & GPIO_IDR_ID0; // 获取IDR的ID0位的值
  ```
* GPIOx_ODR 端口输出数据寄存器。设置引脚的输出值。==如果是输入模式，则是设置上拉或下拉。==
  ```c
  GPIOA->ODR |= GPIO_ODR_OD0; // 配置ODR的0位为1，输出高电平
  ```
* GPIOx_BSRR 端口位设置/清除寄存器，用于间接控制ODR。
  分为BR(reset)和BS(set)两个功能，BR用于清除ODR的位，BS用于设置ODR的位。只写，并且只在写1的时候才起作用。
  ```c
  GPIOA->BSRR |= GPIO_BSRR_BR0; // 配置BR0位为1，清除ODR的0位，输出低电平
  GPIOA->BSRR |= GPIO_BSRR_BS0; // 配置BS0位为1，设置ODR的0位为1，输出高电平
  ```
  **ODR和BSRR寄存器控制输出有什么区别：**
  使用ODR，在读和修改访问之间产生中断时，可能会==发生风险==；BSRR则==无风险==。
  ODR的修改：`GPIOB->ODR |= 1 << 3;` 实际上是`ODR = ODR | (1 << 3)`，先读再改再写三步，如果读出来的时候产生了中断，而中断也会改这里的寄存器，那么中断改了后这里再写，就可能导致冲突。
  BSRR修改：`GPIOB->BSRR = 1 << 3`直接写就行，由于BS只有写1才生效，所以其它GPIO不会受影响。
...
**其它系列的MCU使用的寄存器会有所不同，但是要配置的功能（输入/输出模式、速度、上下拉等）也是这些。如F4/F7/H7系列：**
![图 64](assets/1769130951079.png)  

### 通用外设驱动模型（四步法）
这里不光是GPIO，其它外设也可以使用这个模型。比如 
1. 初始化：时钟设置、参数设置（工作模式等）、IO设置（可选）、中断设置(开中断、设NVIC)(可选)。
2. 读操作（可选）：从外设读取数据。如从串口读取数据，不是必须的。
3. 写操作（可选）：往外设写入数据。如串口输出数据，不是必须的。
4. 中断（可选）：根据中断标志，处理外设各种中断事务。如串口接收中断操作，不是必须的。

### 配置步骤
```c
// 使能时钟（GPIO不用选择时钟源）
RCC->APB2ENR |= RCC_APB2ENR_IOPAEN; // 启用GPIOA时钟。F1系列GPIO挂载在APB2总线上，所以使能APB2时钟。IOPAEN表示GPIOA时钟使能位。
// 下面两行非必须，但是HAL库中是这样操作的。设置时钟后硬件电路需要一些时间才生效，所以读取一下更稳定
volatile uint32_t tmpreg = RCC->APB2ENR & RCC_APB2ENR_IOPAEN; // 设置时钟后读取一下，才能保证时钟稳定的启用，避免后续GPIO操作失效
(void)tmpreg; // 如果声明了tmpreg变量没使用，编译器会警告。这行可防止编译器优化，避免警告

// 配置引脚工作模式（输入模式还可以配置上拉/下拉）
GPIOA->CRL &= ~GPIO_CRL_CNF0;  // 设置PA0的CNF配置位（高2位）为00：通用推挽输出模式
GPIOA->CRL |= GPIO_CRL_MODE0;  // 设置PA0的MODE模式位（低2位）为11：输出模式，最大速度50MHz

// 写操作
GPIOA->ODR &= ~GPIO_ODR_ODR0;  // 将PA0对应的ODR位强制置0 → 输出低电平

// 读操作
uint32_t idr = GPIOA->IDR; // 获取IDR寄存器的值
uint8_t id0 = idr & GPIO_IDR_ID0; // 获取IDR的ID0位的值，即PA0
```
...
### 中断的作用和意义
1. 实时控制：在确定时间内对相应事件作出响应，如：温度监控，通过定时器中断实现每隔1s获取一次温度。
2. 故障处理：检测到故障，需要第一时间处理，如：电梯门夹人了，产生门无法关闭的故障中断，从而可以在中断中通过程序开门。
3. 数据传输：不确定数据何时会来，如：串口数据接收，数据来的时候会产生中断，才开始接受数据。
...
### GPIO外部中断简图
![图 66](assets/1769392660267.png)  

### NVIC
Nested vectored interrupt controller，嵌套向量中断控制器，属于内核部分。中断系统的大总管，接近于内核中用于管理所有中断的一个模块，可以通过 NVIC 配置各种中断类型的优先级。
NVIC支持：256个中断（16内核 + 240外部），256个优先级，允许裁剪（通常都会裁剪）。
==裁剪：只能做减法，从一套完整的资源 / 功能中，移除或禁用不需要的部分，目标是精简资源、降低开销、控制成本。
比如从 256 个中断线里只保留项目需要的 60 个，就是典型的裁剪。==

#### 中断向量表
定义一块固定的内存，以4字节（32位）对齐，存放各个中断服务函数程序的首地址。
中断向量表定义在启动文件，当发生中断，CPU会自动执行对应的中断服务函数。
![图 67](assets/1769408528636.png)  
...
NVIC还有：中断挂起，解挂，激活标志等不常用功能。

NVIC 由于太接近内核，我们一般都使用库函数配置，而不使用寄存器操作的方式。下面的 EXTI 就可以方便的使用寄存器的方式进行配置。

NVIC功能框图：
![图 68](assets/1769477658408.png)  
内核中断能抢占外部中断，外部中断无法抢占内核中断。

#### 中断优先级
优先级值设置的**越小**的，优先级**越高**。中断优先级分为抢占优先级和响应优先级。
...
```c
// 5. NVIC配置示例
NVIC_SetPriorityGrouping(4);    // 全部都是抢占优先级。4->0b011
NVIC_SetPriority(EXTI15_10_IRQn, 4); // EXTI15_10_IRQn 是10-15条线的中断定义
NVIC_EnableIRQ(EXTI15_10_IRQn);
```

#### NVIC使用
1. 设置中断分组：寄存器：SCB_AIRCR[10:8]；调用函数：HAL_NVIC_SetPriorityGrouping
2. 设置中断优先级：寄存器：NVIC_IPRx bit[7:4]；调用函数：HAL_NVIC_SetPriority
3. 使能中断：寄存器：NVIC_ISERx；调用函数：HAL_NVIC_EnableIRQ

**标准库的使用：**
```c
/**
 * @brief  配置EXTI0中断（GPIOA Pin0）的NVIC优先级+使能
 * @param  无
 * @retval 无
 */
void NVIC_Configuration(void)
{
    // -------------------------- 1. 设置中断分组（对应HAL的HAL_NVIC_SetPriorityGrouping） --------------------------
    // 选择分组2：2位抢占优先级，2位子优先级（开发中最常用）
    // 底层操作SCB_AIRCR[10:8] = 010（对应分组2）
    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);

    // -------------------------- 2. 定义NVIC初始化结构体（整合优先级+使能，对应HAL的两步操作） --------------------------
    NVIC_InitTypeDef NVIC_InitStructure;
    
    // 配置中断通道：EXTI0中断（对应GPIOA Pin0的外部中断通道）
    NVIC_InitStructure.NVIC_IRQChannel = EXTI0_IRQn;
    
    // 配置抢占优先级：2位分组下，取值0~3（数字越小优先级越高）
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;
    
    // 配置子优先级：2位分组下，取值0~3
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;
    
    // 使能该中断通道（对应HAL的HAL_NVIC_EnableIRQ）
    // 底层操作NVIC_ISERx寄存器，置位对应位使能中断
    NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
    
    // -------------------------- 3. 应用NVIC配置（核心函数） --------------------------
    // 底层操作NVIC_IPRx（优先级）+ NVIC_ISERx（使能）寄存器
    NVIC_Init(&NVIC_InitStructure);
}

/**
 * @brief  配置GPIOA Pin0为外部中断输入（需配合NVIC）
 * @param  无
 * @retval 无
 */
void EXTI_Configuration(void)
{
    ...
    // 1. 使能GPIOA和AFIO时钟（AFIO用于EXTI引脚映射）
    ...
    // 3. 映射EXTI0到GPIOA Pin0（AFIO_EXTICR1寄存器）
    GPIO_EXTILineConfig(GPIO_PortSourceGPIOA, GPIO_PinSource0);
    
    // 4. 配置EXTI0中断线
    EXTI_InitStructure.EXTI_Line = EXTI_Line0; // 中断线0
    EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt; // 中断模式（非事件模式）
    EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising; // 上升沿触发（可改Falling/Rising_Falling）
    EXTI_InitStructure.EXTI_LineCmd = ENABLE; // 使能中断线
    EXTI_Init(&EXTI_InitStructure);
}

/**
 * @brief  中断服务函数：EXTI0中断处理（必须与中断通道名一致）
 * @param  无
 * @retval 无
 */
void EXTI0_IRQHandler(void)
{
    ...
    if (EXTI_GetITStatus(EXTI_Line0) != RESET)
    {
        ...
        // 清除中断标志位（必须！否则会重复触发中断）
        EXTI_ClearITPendingBit(EXTI_Line0);
    }
}

/**
 * @brief  初始化函数：调用后完成所有配置
 * @param  无
 * @retval 无
 */
void Interrupt_Init(void)
{
    EXTI_Configuration(); // 配置EXTI外设
    NVIC_Configuration(); // 配置NVIC优先级+使能
}
```
...
F1的EXTI包含20个产生事件/中断请求的边沿检测器，即总共：20条EXTI线。
...
|EXTI线23：LPTIM1 异步事件	|	|	|✔|
…			
* GPIO是外部产生的中断，其它都是内部产生的中断/事件。
* F1互联型支持以太网中断，所以是20个，非互联型就只有19个。
* H7有更多的中断和事件，尤其多了很多唤醒事件，因为H7功耗高，为了在低功耗领域也能使用。
...
**相关寄存器：中断屏蔽寄存器（EXTI_IMR）、触发方式寄存器（EXTI_RTSR/EXTI_FTSR）、挂起寄存器（EXTI_PR）、软件触发寄存器（EXTI_SWIER）。**
...
### EXTI和IO映射关系
F1系列使用AFIO进行配置，F4/F7/H7系列使用SYSCFG进行配置。
#### AFIO简介（F1）
Alternate Function IO，即复用功能IO，主要用于重映射和外部中断映射配置。
1. 调试IO配置：`AFIO_MAPR[26:24]`，配置JTAG/SWD的开关状态。（与中断关系不大）
2. 重映射配置：`AFIO_MAPR`，部分外设IO重映射配置。（与中断关系不大）
3. 外部中断配置：`AFIO_EXTICR1~4`，配置EXTI中断线0~15对应具体哪个IO口。（重点）
    `AFIO_EXTICR`的核心作用就是：把你实际想用的 GPIO 引脚（比如 PB3）「映射」到对应的 EXTI 中断线（EXTI3）上。如果不配置这个寄存器，EXTI 中断就不知道该监听哪个 GPIO 引脚（PA3,PB3,PC3都可以连通EXTI3），中断永远不会触发。

**特别注意：配置AFIO寄存器之前要使能AFIO时钟：`__HAL_RCC_AFIO_CLK_ENABLE(); `，对应`RCC_APB2ENR`寄存器 位0**

#### SYSCFG简介（F4/F7/H7）
System configuration controller，即系统配置控制器，用于外部中断映射配置等。
外部中断配置：`SYSCFG_EXTICR1~4`，配置EXTI中断线0~15对应具体哪个IO口。
**特别注意：配置SYSCFG寄存器之前要使能SYSCFG时钟：`__HAL_RCC_SYSCFG_CLK_ENABLE()`，不同系列寄存器有所不同。**

#### EXTI和IO对应关系
![图 70](assets/1769582968185.png)  
我们的目的是要启用某个引脚的中断功能，那么就是要确定分组号和引脚号后，将对应的寄存器使能。
* 相同引脚号的引脚，通往同一个EXTI，比如PA0,PB0,PC0都通往EXTI0，PA1,PB1,PC1都通往EXTI1。
* AFIO_EXTICR1~4的映射：
  |寄存器名|	对应 EXTI 中断线|	寄存器位段（每 4 位对应 1 条中断线）|
  |--|--|--|
  |AFIO_EXTICR1|	EXTI0\~EXTI3|	bits0\~3:EXTI0；bits4\~7:EXTI1；bits8\~11:EXTI2；bits12\~15:EXTI3|
  |AFIO_EXTICR2|	EXTI4\~EXTI7|	bits0\~3:EXTI4；bits4\~7:EXTI5；bits8\~11:EXTI6；bits12\~15:EXTI7|
  |AFIO_EXTICR3|	EXTI8\~EXTI11|	bits0\~3:EXTI8；bits4\~7:EXTI9；bits8\~11:EXTI10；bits12\~15:EXTI11|
  |AFIO_EXTICR4|	EXTI12\~EXTI15|	bits0\~3:EXTI12；bits4\~7:EXTI13；bits8\~11:EXTI14；bits12\~15:EXTI15|
  
  ==每组GPIO引脚有16个，所以EXTI也有16个，这16个EXTI的配置分成了4组，通过AFIO_EXTICR1\~4进行配置。确定要配置的EXTI（引脚号）后选择对应的组，配置相应的位，从而启用相应的分组（分组号）。==
  举例说明：上面的`AFIO_EXTICR1`这一行的 bits0\~3 是4个位，当配置为0000时表示开启PA，又因为0\~3是通往EXTIO的，所以是开启PA0的中断。
  如果要开启PC10的中断，则是需要配置EXTI10，所以需要配置`AFIO_EXTICR3`的8\~11这4位，又因为要开启的是PC，所以8\~11这4位应该设置为0010。
  ==**总结：**==
  ==1. GPIO分为PA,PB,PC...很多组，每一组有0-15个引脚。==
  ==2. AFIO_EXTICR1配置EXTI0\~3，对应0-3引脚；AFIO_EXTICR2配置EXTI4\~7，对应4-7引脚，以此类推。==
  ==3. 确认了要配置的引脚后还要确认分组，每个引脚使用4位来配置要开启的分组（高级MCU分组更多，所以用4位）==
  ==4. 向对应的4位写入要启用分组，如0000启用PA，0001启用PB，0010启用PC，以此类推==
  由于要操作的是`AFIO_EXTICR`寄存器，实际上是16位一起操作（如EXTI0\~3），而我们只会修改其中的某一个（如EXTI3）而其它保持不变，那么就需要使用`|=`操作，而`|=`操作时如果之前EXTI3有值，则会出错，所以得先清空EXTI3的值再写入：
  ```c
  AFIO_EXTICR1 &= ~0xF000; // 清空EXTI3位段（高4位是EXTI3）
  AFIO_EXTICR1 |= 0x2000;  // 写入PC3的编码
  ```

### 中断开发流程
需要根据芯片配置对应的寄存器，根据电路设计配置对应的 GPIO 模式等。
![图 71](assets/1769584154763.png)  
1. 使能GPIO时钟
...
### 中断示例代码
**中断产生后会调用对应的中断服务函数，如`EXTI15_10_IRQHandler`，EXTI10\~15的中断都会进入这个函数，如果PA10和PA11都启用了中断，那么都会调用这个中断函数，还需要自己判断是哪个引脚触发的。**
#### 寄存器方式：
```c
void Key_Init()
{
...
# USART 串口通讯
串口：串行通信接口的简称，指按位发送和接收的接口。如：RS-232/422/485等

### 串行 VS 并行
* 串行：
  ![图 72](assets/1769651511129.png)  
* 并行：
  ![图 73](assets/1769651526108.png)  
...
### 单工/半双工/全双工通信
![图 74](assets/1769651838991.png)  
* 单工通信：数据只能沿一个方向传输
* 半双工通信：数据可以沿两个方向传输，但不能同时进行
* 全双工通信：数据可以同时进行双向传输

### 同步 VS 异步
![图 75](assets/1769651908970.png)  
* 同步：有（同步）时钟信号的是同步通信。如果是单工通信，那么就只需要一根时钟信号线和一根数据信号线。如果是全双工通信那么就需要两根时钟信号线和两根数据信号线。**有了信号线，不再需要设置波特率，通过信号线的信号来同步数据传输。**
* 异步：没有时钟信号的是异步通信，通过特定的电平变化来通知开始和结束。**需要通过波特率来使接收端和发送端的数据同步。**
  像蓝牙等设备，虽然是异步通信，但是不需要手动设置波特率，因为蓝牙设备之间已经内置了同步机制。
...
![图 76](assets/1769675496324.png)  
上图是DB9串口接口，现在已经很少见了，通常都是USB转串口。
DB9通信示意图：
![图 77](assets/1769675724567.png)  
**STM32串口与电脑USB口通信示意图：**
![图 78](assets/1769676317955.png)  
...
#### 波特率
如果传输的数据**都是1**，那么就**一直**是高电平，如果识别到底是发了**多少个1**呢？这时就需要使用波特率了，在二进制中等于比特率。常见的波特率为 4800、9600、115200等。

1. 比特率：每秒钟传送的比特数，单位bit/s
2. 波特率：每秒钟传送的码元数，单位Baud
3. $比特率 = 波特率 * log_2^M$ ，M表示每个码元承载的信息量（进制数）
...
# DMA 直接存储访问
直接存储器存取（direct memory access，DMA）用来提供在**外设和存储器之间或者存储器和存储器之间**的高速数据传输。无须CPU干预，数据可以通过DMA快速地移动，这就节省了CPU的资源来做其他操作。
2个**DMA控制器**有12个通道（DMA1有7个通道，DMA2有5个通道），每个通道专门用来管理来自于一个或多个外设对存储器访问的请求。还有一个**仲裁器**来协调各个DMA请求的优先权。
DMA控制器和Cortex™-M3核心共享系统数据总线，执行直接存储器数据传输。当CPU和DMA同时访问相同的目标（RAM或外设）时，DMA请求会暂停CPU访问系统总线达若干个周期，总线仲裁器执行循环调度，以保证CPU至少可以得到一半的系统总线（存储器或外设）带宽。（DMA会优先占用总线访问权，从而导致CPU不能使用总线，但是总线仲裁器会通过调度保证CPU至少可以占用一半的时间）
要注意的是DMA2只存在于大容量产品和互联型产品中。
![图 0](assets/1761878214092.png)  

1. DMA 请求：如果外设要想通过DMA来传输数据，必须先给DMA控制器发送DMA请求，DMA控制器收到请求信号之后，控制器会给外设一个应答信号，当外设得到控制器的应答信号后，外设会立即释放它的请求。（控制器不应答，就会一直发请求，不释放-停止）
2. 通道：DMA具有多个独立可编程的通道，每个通道对应不同的外设的DMA请求。虽然每个通道可以接收多个外设的请求，但是同一时间只能接收一个，不能同时接收多个。
3. 仲裁器：当发生多个DMA通道请求时，就意味着有先后响应处理的顺序问题，这个就由仲裁器管理。
4. 传输方向：存储器到外设，外设到存储器，存储器到存储器。这里的存储器指的是ROM和RAM。注意DMA只能ROM到RAM，没有办法把数据从RAM传输到ROM（flash）。
...
    DMA1_Channel1->CCR &= ~DMA_CCR1_DIR; // 从外设读（ROM在这里是当成外设看待）
    DMA1_Channel1->CCR |= DMA_CCR1_MEM2MEM; // 从存储器到存储器（ROM到RAM）
```
3. 设置外设和存储器宽度：通常两个设置成相同的值，方便传输和接收
```c
    /* 4. 设置存储器和外设数据宽度 00：8位  01：16位  10：32位*/
    DMA1_Channel1->CCR &= ~DMA_CCR1_MSIZE; // 存储器数据宽度00：8位
    DMA1_Channel1->CCR &= ~DMA_CCR1_PSIZE; // 外设数据宽度00：8位
```
4. 设置外设和存储器地址自增：比如传输了1位数据后，地址自增1，传输下一个数据时就会自动放到下个地址
```c
    /* 5. 外设和存储器地址自增 0:不自增 1：自增*/
    DMA1_Channel1->CCR |= DMA_CCR1_MINC; // 存储器地址自增
    DMA1_Channel1->CCR |= DMA_CCR1_PINC; // 外设地址自增
```
5. 数据传输长度：每传输完一个字节，CNDTR寄存器就会减1，直到减到0，就表示传输完成。传输完成会产生一个中断。
```c
    /* 配置要传输的数据长度 */
    DMA1_Channel1->CNDTR = dataLength; // dataLength为0-65535，16位
```
6. 传输中断使能
```c
    /* 6. 开启传输完成中断使能 */
...
```
7. 配置源和目的地地址：数据传输的基地址，没传输完一个字节，就会自动增加1，这样进行传输。
```c
    /* 配置外设地址 */
...
```
### 总结
DMA是比较底层的部分，类似中断在MCU中的地位，会和各种外设通信，比如UART、SPI、I2C等。所以这些外设都有DMA相关寄存器来配置DMA传输。（各种外设也有相应的中断）
...
# SPI 通信
### 物理层
SPI，Serial Peripheral interface，串行外围设备接口。SPI接口主要应用于EEPROM，FLASH，各种传感器，AD转换器等。SPI是一种高速的，全双工，同步的串行通信总线。
1个SPI设备一般有4条线
* SCK：时钟信号线，用于通讯数据同步。它由通讯主机(MCU)产生，决定了通讯的速率，不同的设备支持的最高时钟频率不一样，如 STM32 的 SPI时钟频率最大为 fpclk/2（外设总线频率的一半）。
* MOSI：主设备输出，从设备输入信号线，用于通讯数据发送。
* MISO：主设备输入，从设备输出信号线，用于通讯数据接收。
* SS：片选线或者使能线。有时候也称NSS（低电平有效）或CS。SPI通信时各个设备是没有地址的，主机和哪个从机通讯，就把谁的SS置为低电平（主设备和每个从设备都通过一条SS线连接）。如果主机要连接多个从设备就需要多条片选线（此时就不止4条线了）。
  ![图 5](assets/1762507371983.png)  
只能主从设备之间进行通信，从与从之间是无法进行通信的。
### 协议层
#### 主机和从机之间的数据交换
![图 6](assets/1762565176006.png)  
...
最后从接收缓冲区就可以读出数据了。
#### 时钟的极性和相位
什么时候读数据(数据采样)，和设置的时钟极性和相位有关。
* 时钟极性（CPOL clock polarity）：通信的整个过程分为空闲时刻和通信时刻。时钟信号在空闲状态时的电平。
  * 空闲状态 SCK 是低电平，CPOL=0
    ![图 7](assets/1762565698160.png)  
  * 空闲状态SCK 是高电平，CPOL=1
    ![图 8](assets/1762565749593.png)  
* 时钟相位（CPHA clock phase）：直接决定SPI总线从哪个跳变沿开始采样数据。数据采样的时间点。
  * CPHA=0：表示从第一个跳变沿开始采样;
  ![图 9](assets/1762565955215.png)  
  * CPHA=1：表示从第二个跳变沿开始采样;
  ![图 10](assets/1762565988332.png)  
  
时钟极性和相位的组合可以有4种情况：
* 00：时钟空闲时为低电平，数据在时钟上升沿采样。
* 01：时钟空闲时为低电平，数据在时钟下降沿采样。
* 10：时钟空闲时为高电平，数据在时钟上升沿采样。
* 11：时钟空闲时为高电平，数据在时钟下降沿采样。
一般使用00或10这两种组合，都是在上升沿采样数据。用得最多的还是00。
![图 12](assets/1762567802127.png)  
![图 13](assets/1762568034458.png)  
### W25Q32Flash芯片
#### 引脚连接
![图 14](assets/1762583841697.png)  
![图 15](assets/1762584586712.png)  
#### Flash芯片存储结构
1. 1个字节对应1个地址，4MB的flash芯片就有4MB个地址，即$2^{22}$，需要22位来表示地址。可以使用6个16进制数来表示地址，每个16进制数占用4位，即最高可表示24位，当然22位也就能表示了。3FFFFF就是22位的最大值。
2. 将4MB的flash分为64个块（block），每个块有64KB的大小（$2^{22}÷2^{6}=2^{16}$即$4MB÷64=64KB$）。每块$2^{16}$用16进制可表示为FFFF，FFFF+1就成了下一块的起始地址10000，所以每块的地址就是从xx0000-xxFFFF，xx就可以看作是块号，后面4位看作块内地址。
3. 直接操作64KB的空间也比较麻烦，所以做进一步的划分。将每一块分为16个扇区（$2^{16}÷2^{4}=2^{12}$），每个扇区有4KB的大小。每个扇区$2^{12}$用16进制可表示为FFF，FFF+1就成了下一个扇区的起始地址1000，所以每个扇区的地址就是从xxx000-xxxFFF，xxx中的第三位就可以看作是扇区号（前两位是块号），后面3位看作扇区内地址。
4. 再将每个扇区分为16页(page)（$2^{12}÷2^{4}=2^{8}$），每页256B的大小。每页$2^{8}$用16进制可表示为FF，FF+1就成了下一页的起始地址100，所以每页的地址就是从xxxx00-xxxxFF，xxxx中的第四位就可以看作是页号（前3位是块号和扇区号），后面2位看作页内地址。最后就得到xxyzdd，其中xx是块号，y是扇区号，z是页号，dd是一个页内的数据共8位256字节。
5. 每次写入就是进行页写，需要先擦除再写。但是擦除的最小单位是扇区，需要注意这点。
   ![图 16](assets/1762589063805.png)  
##### 写入操作注意事项
1. 写入操作前，必须先进行写使能。
2. 写入数据前必须先檫除，檫除后，所有数据位变为1。擦除必须按最小擦除单元进行（这里就是扇区擦除）。
3. 每个数据位只能由1改写为0，不能由0改写为1。
...
## ESP32
ESP32-C3本身就可以作为一个单片机使用（需要单独学习如何开发），也可以作为一个 WiFi/蓝牙 模块使用，连接到主控单片机上。STM32与ESP32-C3使用串口进行通讯。STM32可以给ESP32-C3发送命令，这种命令叫ESP-AT指令。
ESP-AT 提供了大量功能不同的 AT 命令，如 Wi-Fi 命令、TCP/IP 命令、Bluetooth LE 命令、Bluetooth 命令、MQTT 命令、HTTP 命令、Ethernet 命令等。
![图 27](assets/1763084328061.png)  
...
### 固件烧录
ESP32默认状态下是没有固件程序的，所以要使用时，需要先烧写固件程序。
![图 28](assets/1763085190657.png)  

点 OK 后弹出下图：
![图 29](assets/1763085465356.png)  

烧录：
![图 30](assets/1763086275124.png)  
> 注意：首次烧录时，插上USB后，设备管理器可能会一直在闪，不过也能点开端口观察新增的COM是多少。然后点击START也可能出现无法识别串口等提示，多点几次START，卡在它能正确识别的时间开始烧写固件，就能成功烧录。烧录成功后设备管理器也不会再闪了。
...
# 蓝牙
蓝牙协议包括两种技术：BR（Basic Rate 经典蓝牙）和LE（Low Energy 低功耗蓝牙）。
这两种技术都包括搜索（discovery）管理、连接（connection）管理等机制，但它们是相互独立的，不能互通的技术！（比如搜索功能，BR和LE都要单独实现，而不是互通的）

### 蓝牙架构
1. SOC蓝牙单芯片方案：一般是半导体厂商半开源协议栈，把开发的蓝牙协议栈直接烧写到蓝牙芯片中。此类芯片一般可以直接做为MCU用，这类产品一般用于消费类电子，集成度很高，调一调参数可以直接使用，常见的有蓝牙耳机等产品。
   ![图 17](assets/1762761122744.png)  
2. SOC蓝牙+MCU方案：在集成好的蓝牙芯片基础上，通过特定的接口（UART居多），发送自定义的command来达到想要的功能，比如发送0x01代表搜索周围设备。
  外设一个单芯片方案，其自定义指令包含蓝牙芯片（BT Controller）、微控制单元（MCU）以及蓝牙协议栈（BT Host）。
  此部分的应用，将蓝牙作为一个外设使用，用于远程通信，例如网上卖的一些蓝牙串口。
   ![图 18](assets/1762761188120.png)  
3. 蓝牙host+controller分开方案：这种应用算是蓝牙最复杂的应用，客户需要使用蓝牙的场景有很多，涉及的蓝牙协议也有很多，需要将Host与Controller分开，集成更多的蓝牙协议，比如蓝牙电话（HFP）、蓝牙音频（A2DP）、蓝牙音乐控制（AVRCP）、蓝牙电话本（PBAP）、蓝牙短信（MAP）等。
   ![图 19](assets/1762761364856.png)  

### 蓝牙协议栈
蓝牙协议很复杂，类似TCP/IP协议，分为多层，这种层层封装的协议就叫做协议栈（protocol stack）。
#### 蓝牙芯片架构
...
* LL：设备可以划分为主机和从机，从机广播，主机可以发起连接。
  等待被连接的是从机（单片机），主动发起连接的是主机（手机）。
* GAP：定义了4种特定角色：广播者、观察者、外围设备和中心设备。
* GATT：设备可以分为服务端和客户端。
  被连接的设备是服务器，发起连接的设备是客户端。所以单片机上的蓝牙是服务器，服务器上拥有我们需要的信息，而手机是客户端，客户端需要连接服务器才能获取信息。
### BLE的地址
分为公共地址和随机地址。**这是两种类型的地址，而不是前面是公共地址后面跟上随机地址**。蓝牙芯片选择其中一种类型来使用即可，由于公共地址需要申请购买，现在一般都是使用随机地址。
#### 公共地址
BLE的公共地址，就类似于我们日常的身份证号码，是全球唯一的且不可改变的（类似MAC地址）。
为了保证BLE公共地址的全球唯一性，其需要向IEEE购买，然后IEEE组织就会对应地分配公共地址给买家。
总长度为6个字节，共48位。
![图 22](assets/1762765227694.png)  
#### 随机地址
除了公共地址类型之外，还有一个随机地址类型，提供额外的隐私保护，其又分为静态地址（Static Address）和私有地址（Private Address），它们之间主要通过最高的2位有效位来区分。
1. 静态地址。总长度也是48位，但是最高位的2位必须是0b11。随机部分至少有一个位是0和1。也就是说不能全部是0或全部是1。**设备重启之前不会改变**。物联网设备大多很少断电，所以静态地址相对也没那么安全。
![图 23](assets/1762765347397.png)  
2. 私有地址。总长度也是48位。最高位是00/01，00表示不可解析的私有地址；01表示可解析的私有地址，通过密钥和hash算法等生成地址。随机部分至少有一个位是0和1。也就是说不能全部是0或全部是1。**定时更新改变**。
![图 24](assets/1762765378053.png)  

随机地址说白了就是厂商白嫖，因为地址是免费的。当然也提高了隐私。随着隐私和安全性的关注增加，越来越多的蓝牙设备开始使用随机地址来减少被追踪的风险。这在蓝牙低功耗（BLE）技术中尤为常见。

### 通讯
#### 广播
广播是指从机（服务器）每经过一个时间间隔发送一次广播数据包，这个时间间隔称为广播间隔，这个广播动作叫做广播事件，只有当从机处于广播状态时，主机（客户端）才能发现该从机（服务器）。
在每个广播事件中，广播包会分别在37、38和39三个信道上依次广播（可选择只在一个或两个信道上广播），如下图所示。
![图 25](assets/1762766534317.png)  
还可以通过设置让广播后是否允许扫描的设备连接或扫描到等。
...
从机作为GATT的Server端，用来定义和存储Profile。Profile包含一个或者多个Service，每个Service又包含一个或者多个Characteristic（特征），Characteristic是主从通信的最小单元。
主机作为GATT的Client端，用来发现和获取从机的Service和Characteristic，从而与之通信。
![图 26](assets/1762767013179.png)  
关于服务 Service和特征值Characteristic的概念。每个服务和特征值都有自己的唯一标识 UUID，标准UUID为128位，蓝牙协议栈中一般采用16位，也就是两个字节的UUID格式。
一个从机设备包括一个或者多个服务；一个服务中又可以包括一条或者多条特征值，每个特征值都有自己的属性 Property，属性的取值有：可读 Read、可写 Write、通知 Notify（类似UDP，不需要回复）、指示Indicate（类似TCP，需要回复）。

### 透传模式
蓝牙传输有几种模式：
* 命令模式：可以发送AT命令
* 数据模式：通过特定的命令可以进入数据模式（如使用命令`AT+BLESPP=ID,LEN`进入数据模式），数据模式传输的数据长度需要在输入命令时指定，所以限制较多。传输的数据达到指定的长度后就会自动退出数据模式，返回命令模式。
* 透传模式：透明传输，不管数据长度等（如使用命令`AT+BLESPP`进入透传模式，使用`+++`可退出透传模式进入命令模式）
...
AT+BLEINIT=2
// 2.服务端创建服务
AT+BLEGATTSSRVCRE  // BLE GATT S SRV CRE
// 3.开启全部服务
AT+BLEGATTSSRVSTART  // BLE GATT S SRV START
// 4.获取MAC地址
AT+BLEADDR?  // BLE ADDR
// 5.设置广播参数：最小广播间隔，最大广播间隔，广播类型，BLE地址类型，广播信道，广播过滤器规则...
AT+BLEADVPARAM=50,50,0,0,7,0 // BLE ADV PARAM
// 6.设置广播数据：使用16进制字符串
AT+BLEADVDATA="1542A4E6DF35" // 这个不方便，一般使用下面那种，可读性更强
// 7.设置广播数据扩展方式：设备名称，设备UUID（随便设置），制造商数据（随便设置）
AT+BLEADVDATAEX="test","A001","11223344" // BLE ADV DATA EX
// 8.服务端开始广播
AT+BLEADVSTART  // BLE ADV START

// 9.服务端查询特征
AT+BLEGATTSCHAR? // BLE GATT S CHAR
// 会响应这样的内容，最后一个是服务端的特征值
...
+BLEGATTSCHAR:"char",1,3,0xC302,0x08 // 0x08 表示 write
+BLEGATTSCHAR:"desc",1,3,1,0x2901
+BLEGATTSCHAR:"char",1,4,0xC303,0x04 // 0x04 表示 write without response
+BLEGATTSCHAR:"desc",1,4,1,0x2901
+BLEGATTSCHAR:"char",1,5,0xC304,0x08
+BLEGATTSCHAR:"char",1,6,0xC305,0x10 // 0x10 表示 notify
+BLEGATTSCHAR:"desc",1,6,1,0x2902
+BLEGATTSCHAR:"char",1,7,0xC306,0x20 // 0x20 表示 indicate
...
所以客户端可通过 0x08 和 0x04 来往服务端写入数据，服务端可通过 0x10 和 0x20 通知客户端数据变化。

// 10.服务端配置BLE SPP：1-设置SPP参数，tx服务序号，tx服务特征序号，rx服务序号，rx服务特征序号
AT+BLESPPCFG=1,1,7,1,5
tx配置为了1和7，表示选择了 +BLEGATTSCHAR:"char",1,7,0xC306,0x20，这是个 indicate 类型的特征，对应发送
rx配置为了1和5，表示选择了 +BLEGATTSCHAR:"char",1,5,0xC304,0x08，这是个 write 类型的特征，对应接收。

// 11.配置在透传模式下，连接发生变化时会传输一个提示信息
// 4表示二进制0100，把位2设置为1，当WIFI、蓝牙连接状态发生变化时，会发送一个提示信息
AT+SYSMSG=4 

// 12.服务端使能 BLE SPP：进入透传模式
AT+BLESPP
```
...
## 江协科技蓝牙讲解
[江协科技蓝牙模块&蓝牙串口小程序 使用教程](https://www.bilibili.com/video/BV1aDndzEEk1/?vd_source=8220e726dcb3a350fd156cea947bd58b)
### 简介
蓝牙模块使用的是串口通信，可以看作是一个串口转蓝牙传输功能的模块。
#### 蓝牙类别：
* 经典蓝牙：SPP，蓝牙3.0及以下版本使用。功耗高，速率较快，先配对后连接。（iOS都不支持）
* 低功耗蓝牙：BLE，蓝牙4.0及以上版本使用。功耗低，速率较慢（随着版本更新迭代，已经相差无几了），快速连接，增加组网、广播、定位等新功能。
* 双模蓝牙：同时支持经典蓝牙和低功耗蓝牙。
...
### 硬件连接
![图 1](assets/1762147744081.png)  

### 传输模式
* AT指令模式：通过发送AT指令来配置蓝牙模块的参数，如设置蓝牙名称、波特率、连接模式等。
* 串口模式：通过串口与蓝牙模块进行数据通信，发送和接收数据。

AT模式实际上也是通过串口来发送和接收AT指令的，只是AT指令的格式和数据是固定的，而串口模式可以发送和接收任意数据。
当蓝牙模块连接到蓝牙设备后，通常会自动进入串口模式，发送接收的数据会进行透传。

##### 使用串口工具发送AT命令：
![图 2](assets/1762148191102.png)  
具体的AT指令可以参考蓝牙模块的用户手册。

##### 手机蓝牙测试APP
![图 3](assets/1762150511431.png)  
评论区有推荐 nrf connect，功能比较专业。

![图 4](assets/1762151098820.png)  
发送接收模式还分为文本模式和HEX模式，在进行蓝牙功能开发时通常会用到HEX模式来发送接收数据进行解析。
...
STM32 有2个看门狗，独立看门狗和窗口看门狗。独立看门狗号称宠物狗，窗口看门狗号称警犬。

### 独立看门狗
IWDG（Independent watchdog，独立看门狗）。独立看门狗用通俗一点的话来解释就是一个 12 位的递减计数器，**当计数器的值从某个值一直减到 0 的时候，系统就会产生一个复位信号，即 IWDG_RESET**。如果在计数没减到 0 之前，**刷新了计数器的值的话**，那么就不会产生复位信号，这个刷新计数器值的动作就是我们经常说的喂狗。
**就是一个计数器，程序正常运行时我们需要在某个节点刷新计数器的值，防止它减到0从而复位系统。**
![图 31](assets/1763435897300.png)  
#### 计数器时钟
独立看门狗的时钟由独立的 RC 振荡器 LSI 提供，即使主时钟发生故障它仍然有效，非常独立（叫独立看门狗的由来）。
独立看门狗的定时时间并不精确，只适用于对时间精度要求比较低的场合。
#### 预分频寄存器
存储着我们想要的预分频值。
![图 32](assets/1763436203726.png)  
#### 12位递减计数器
...
![图 33](assets/1763436285914.png)  
这里的超时时间是指如果超过这个时间不喂狗，就会产生复位信号。
超时时间的计算公式：$T_{IWDG}=\frac{1}{时钟频率} \times 预分频系数 \times (RL+1)$
注意：
1. 时钟频率是40KHz。
2. 预分频系数只有表中固定的这些值（3位PR值决定）。
3. RL的值就是重装载寄存器中的值。计数器的初始值。
#### 键寄存器
键寄存器 IWDG_KR 独立看门狗的一个**控制寄存器**，用于操作独立开门狗，只能写入0-15位，读永远是0。三种控制方式，往这个寄存器写入下面三个不同的值有不同的效果。
![图 34](assets/1763436798638.png)  
#### 状态寄存器
状态寄存器 SR 只有位 0：PVU 和位 1：RVU 有效，这两位只能由硬件操作，软件操作不了。
![图 36](assets/1763451426724.png)  

### 窗口看门狗
WWDG（Window watchdog，窗口看门狗）。窗口看门狗跟独立看门狗一样，也是一个递减计数器不断的往下递减计数。
当计数器减到一个固定值 0x40（即0100 0000，T6位为1。注意这个寄存器的最高位是使能位）时还不喂狗的话（T6位的1变成0），产生复位，这个值叫窗口的**下限**，是固定的值，不能改变。这个是跟独立看门狗类似的地方。
不同的地方是窗口看门狗的计数器的值在减到某一个数之前喂狗的话也会产生复位，这个值叫窗口的**上限**，上限值由用户独立设置。窗口看门狗计数器的值必须在**上窗口和下窗口之间**才可以喂狗，喂早了喂晚了都不行。这就是窗口看门狗中窗口两个字的含义。
![图 41](assets/1763453183645.png)  
#### 时钟来源
来源于PCLK1，PCLK1的最大频率是36MHz，由RCC控制开启。
#### 时钟频率
计数器时钟由 CK 计时器时钟经过预分频器分频得到，分频系数由配置寄存器WWDG_CFR的[8:7]两位（WDGTB）配置，可以是 [0,1,2,3]。
所以$计时器时钟频率 = PCLK1/4096/分频系数$，这里先除以4096是因为PCLK1在进来之前有一个4096的分频，只是图中没有画出来而已。
![图 38](assets/1763452510602.png)  
![图 39](assets/1763452582070.png)  
#### 计数器
...
实际上真正用来计数的是 T[5:0]。当递减计数器递减到 0X40 的时候，还不会马上产生复位，如果使能了提前唤醒中断：CFR 位 9 (EWI) 置 1，则产生提前唤醒中断，如果真进入了这个中断的话，就说明程序肯定是出问题了，那么在中断服务程序里面我们就需要做最重要的工作，比如保存重要数据，或者报警等，这个中断我们也叫它**死前中断。**
![图 40](assets/1763452663394.png)  
#### 窗口值
窗口值要根据代码运行时间来设置。当代码运行太快（可能部分逻辑没执行到），或当代码运行太慢（可能卡死了）时，都应该产生复位。
一般计数器的值设置成最大 0x7F，窗口值为 WR，计数器减一个数的时间为 T，那么时间：（0x7F-WR）*T 应该稍微大于 Ta 即可，这样就能做到刚执行完程序段 A 之后喂狗，起到监控的作用，这样也就可以算出 WR 的值是多少。
![图 42](assets/1763454013580.png)  

### 独立看门狗和窗口看门口的对比
![图 43](assets/1763454214072.png)

### 寄存器
验证独立看门狗是否能复位程序。假设程序正常执行需要3s，执行完毕之后喂狗，我们把超时时间设置为4s。当检测到按键1按下后，我们延长3s，那么喂狗超时，就会产生复位。
1. 启动开门狗
```c
    /* 1. 启动看门狗.看门狗一旦启动就无法停止.  kR=0xCCCC 会自动开启 LSI */
    IWDG->KR = 0xCCCC;

    /* 2. 允许访问预分频寄存器和重装载寄存器 KR=0x5555 */
    IWDG->KR = 0x5555;
```
2. 配置预分频和重装载寄存器
假设超时时间T是4s即4000ms，LSI频率是40KHz，那么可以计算出预分频系数和重装载寄存器的值。
$T_{IWDG}=\frac{1}{40000} \times 64 \times (2499+1)$
```c
    /* 3. 写入预分频值 100 即 4，64分频 */
    IWDG->PR = 4;
    /* 4. 写入重装载值 */
    IWDG->RLR = 2499;
```
