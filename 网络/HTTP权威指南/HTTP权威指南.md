...
> 关于跨域：跨域请求时，如果是简单请求，浏览器会直接将请求发送给服务器，而服务器会接收到请求并返回数据。但是没有配置 CORS 等跨域解决方案，浏览器接收到数据后会将数据过滤掉，所以我们获取不到服务器返回的数据。而复杂请求浏览器就会先发送个 OPTION 请求，这时服务器也会根据是否配置了跨域方案来进行处理，如果不允许跨域则返回拒绝信息，并不会将数据返回。
...
### UDP
* UDP 是无连接的，发送数据前不需要进行3次握手。
* UDP 支持一对多的方式，所以拥有单播，多播，广播的功能。
* 不可靠，因为不需要建立连接，即使无法连接上也会发送数据；也没有拥塞控制等。

##### UDP 的优势
* UDP 效率更高，从上面的特点也能看出，所以对实时性要求较高的场景可以使用 UDP 提升性能。
* 不在乎丢包的场景，如直播、视频通话等。
* 包很小，不需要分包，只需要重发即可。如 DNS协议。

**TCP 就像打电话，需要对方接听才能通讯；UDP 就像发短信，只管自己发过去就行，还能群发。**
...
在这时间内重复访问，将触发 **强缓存**，响应状态码为 200 from cache.
...
强缓存时间过期后，再次请求才会触发这里的再验证，这时就需要服务器判断资源是否有变化，没有变化就会返回 304 not modified，这叫做 **协商缓存**。
...
## keep-alive
TCP/IP 的 Keepalive（保活机制）是一个非常实用但也容易被误解的概念。简单来说，它确实会发送“心跳”，但这个心跳是**由内核维护的、不包含应用数据的探测报文**。

以下是该机制的详细工作原理：

### 1. Keepalive 的工作流程

当一个 TCP 连接建立后，如果双方长时间没有数据传输，Keepalive 机制就会启动。它的逻辑如下：

1. **静默期等待**：如果在一段指定的时间（默认通常是 2 小时）内，连接上没有任何数据往来，一方（通常是服务端）的内核就会尝试发送一个**探测报文**（Keepalive Probe）。
2. **发送探测包**：这个报文是一个高度简化的 TCP 包，它的序列号（Sequence Number）被设置为比当前确认号少 1，且不携带任何应用层数据。
3. **对方响应**：
* **正常运行**：如果对方收到并回复了 ACK，说明连接依然健康，计时器重置，重新进入等待期。
* **对方崩溃/宕机**：如果对方没响应，系统会每隔一段时间（如 75 秒）重复发送，直到达到最大重试次数。
* **重置连接**：如果达到重试上限依然无果，系统会认为连接已断开，并发送一个 RST 包关闭连接，通知应用程序。

### 2. 关键参数（内核级别）

在 Linux 系统中，Keepalive 的行为由以下三个参数控制：

| 参数名 | 默认值 | 说明 |
| --- | --- | --- |
| `tcp_keepalive_time` | 7200 秒 (2小时) | 最后一次数据传输后，多久开始发送探测包。 |
| `tcp_keepalive_intvl` | 75 秒 | 探测包没有回复时，下一次探测的间隔时间。 |
| `tcp_keepalive_probes` | 9 次 | 连续探测失败多少次后，认定连接失效。 |

### 3. 它与“应用层心跳”的区别

很多人会混淆 TCP Keepalive 和 WebSocket/IM 软件里的“心跳包”。它们的主要区别在于：

* **TCP Keepalive (内核态)**：
    * **省流量**：由操作系统自动完成，不涉及应用逻辑。
    * **灵活性差**：默认时间太长（2小时），且它是基于连接的，不能检测应用层是否“假死”（例如进程锁死但内核还在）。
    * **目的**：主要为了回收长期空闲的死连接，或者防止防火墙/NAT 网关因为连接长时间不活动而将其断开。

* **应用层心跳 (用户态)**：
    * **更精准**：由开发者自己写代码实现（如 Ping/Pong 报文）。
    * **检测全面**：不仅能检测网络通不通，还能检测对方程序是否还能处理业务逻辑。
    * **建议**：在需要高可靠性的场景（如即时通讯、长连接推送）中，通常会**禁用或辅助使用** TCP Keepalive，而主要依靠应用层心跳。
